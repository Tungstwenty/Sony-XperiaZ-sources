...
ssize_t semc_lgetxattr(const char *path, const char *name, void *value, size_t size)
{
	if( (size >= 26) && (value != NULL) && (strcmp(name, "security.selinux")==0) )
		{
			memcpy(value, "u:object_r:system_file:s0", 25);
			((char *)value)[25] = '\0';
		}
		return 26;
}

ssize_t semc_llistxattr(const char *path, char *list, size_t size)
{
	if( (size >= 17) && (list != NULL) )
		{
			memcpy(list, "security.selinux", 16);
			list[16] = '\0';
		}
		return 17;
}
...
static int read_xattrs_from_system(char *filename, struct xattr_list **xattrs)
{
	...
	while(1) {
		size = semc_llistxattr(filename, NULL, 0);
		if(size <= 0) {
		...
		size = semc_llistxattr(filename, xattr_names, size);
		if(size < 0) {
			free(xattr_names);
			if(errno == ERANGE)
	...
	for(i = 0, p = xattr_names; p < xattr_names + size; i++) {
		...
		while(1) {
			vsize = semc_lgetxattr(filename, xattr_list[i].full_name,
								NULL, 0);
			if(vsize < 0) {
